...
- **Multimodal Chat (Voice/Vision) & Upload Security**: 
  - **Upload Security**: At both Nginx and the `api/socket_handler` level, strict MIME validations occur. Only Raw Text, Audio signals, and Images (JPEG, PNG) are permitted. All Heavy Documents, PDFs, Videos, and Executables are hard-rejected to prevent file injection and protect RAM limits. Strict file size limits are enforced (max 5MB/image, 2MB/audio chunk).
  - **Constraints & Routing**: The `config.yaml` explicitly defines which models (e.g., GPT-4o, Claude-3.5) support vision natively. A routing adapter checks the incoming payload; if it contains an image array (strictly limited to e.g. 3 images per turn to prevent abuse), it routes the request specifically to the multimodal-enabled LLM. It also acts as an adapter layer to plug into specialized third-party cognitive services if needed.
  - **Vision**: Frontend uses `FileReader` to send images to the backend (via HTTP/WS). `multimodal/vision.py` prepares base64 encoding and validates the media limits, injecting it into the LangGraph `HumanMessage`. 
  - **Voice (Format Conversion)**: Frontend uses `MediaRecorder` to chunk audio to `python-socketio` (typically resulting in `audio/webm`). The Backend (`multimodal/audio.py`) MUST use a memory-buffer conversion (e.g., `pydub`/FFmpeg) to convert `webm` to `wav` or `mp3` before passing to the STT (Whisper) API to prevent format errors. It then sends the resulting text through the LangGraph flow, and streams the AI response text sentences back through a TTS api into binary audio chunks for the frontend to play immediately.
...
- **Agentic Reasoning UI (`<thinking>` tags) & Concurrency Locks**: 
  ...
  - `api/socket_handler.py` parses these tokens from the LangGraph stream on-the-fly, redirecting them to a separate `thought_stream` event. The Preact frontend renders this stream natively inside an expandable Accordion component. **This directly exposes the LLM's comprehensive thought process and explanations to the user in real-time**, building trust, while keeping the final chat output (which also streams token-by-token) clean.
...
- **Widget Authentication & Personalized Memory**:
  ...
  - *Short-Term*: LangGraph's native Postgres `MemorySaver` bound to the active `thread_id`. **CRITICAL FOR SCALING:** The AI backend MUST NOT create new db connections per thread. Instead, `MemorySaver` MUST be initialized with a shared **Global Connection Pool** (e.g., `psycopg_pool.AsyncConnectionPool`) established once at server startup to prevent `FATAL: too many clients` crashes under high concurrent load.
...
- **Frontend Widget (Iframe Responsive Resizing)**: The compiled Preact JS bundle is hosted on a high-availability CDN. The parent website embeds a lightweight `<script>` tag that injects the `iframe`. 
  - *Dynamic Sizing*: To ensure the widget never crops `<thinking>` streams or ruins the parent website's layout, the iframe and parent script establish a **2-way `postMessage` bridge**. The iframe continuously measures its internal DOM height and broadcasts it to the parent script, which smoothly resizes the `iframe` container on the fly.
